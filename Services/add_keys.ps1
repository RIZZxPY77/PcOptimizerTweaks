...
$newKeys = @{
    ...
    "GameDetail_WarningText" = @{
        ...
        "ja" = "ゲーム設定とプロ設定は互換性が制限されています。これらの機能はすべてのゲームで機能するとは限りません。";
    ...
    "GameMode_OptRemovedText" = @{
        "id" = "Profil default dikembalikan untuk {0}"; "en" = "Default profile restored for {0}"; "ja" = "{0}のデフォルトプロファイルが逆転されました"; "zh" = "“{0}”已恢复默认配置"; "ar" = "تم استعادة ملف التعريف الافتراضي لـ {0}"; "pt" = "Perfil padrão restaurado para {0}"; "es" = "Perfil predeterminado restaurado para {0}"; "ru" = "Восстановлен профиль по умолчанию для {0}"; "de" = "Standardprofil für {0} wiederhergestellt"; "fr" = "Profil par défaut restauré pour {0}"; "ko" = "{0}에 대한 기본 프로필이 복원되었습니다."; "tr" = "{0} için varsayılan profil geri yüklendi"
    }
    ...
    "Emu_Desc_NoxPlayer" = @{
        ...
        "es" = "Aumenta la asignación de RAM del proceso del emulador y reduce la latencia de entrada del mouse y teclado.";
...
$newRawJson = ConvertTo-Json $data -Depth 20
[System.IO.File]::WriteAllText($jsonPath, $newRawJson, [System.Text.Encoding]::UTF8)
Write-Output "Successfully updated translations.json via PowerShell!"
